...
pub const InstrFormat = enum { unknown, R, R4, RAtomic, I, IShift, IFence, S, B, U, J, None };
...
pub fn instrData(tag: InstrName) InstrSpec {
    return instrs[@intFromEnum(tag)];
}
...
pub const instrs: []const InstrSpec = &[_]InstrSpec{
    ...
    .{ .name = "SLL", .format = .R, .opcode = 0b0110011, .funct3 = 0b001, .funct7 = 0b0000000, .banks = .{ .rs1 = .uint, .rs2 = .sint, .rd = .uint } },
    ...
    .{ .name = "SRL", .format = .R, .opcode = 0b0110011, .funct3 = 0b101, .funct7 = 0b0000000, .banks = .{ .rs1 = .uint, .rs2 = .sint, .rd = .uint } },
