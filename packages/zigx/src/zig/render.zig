// BEGIN EDIT
// all edits are surrounded with this, for easy identification & updating
// END EDIT
...
pub const Fixups = struct {
    ...
    // BEGIN EDIT
    /// false: render to zigx, true: render to zig
    render_to_zig: bool = false,
    // END EDIT
...
fn renderExpression(r: *Render, node: Ast.Node.Index, space: Space) Error!void {
    ...
    switch (node_tags[node]) {
        ...
        // BEGIN EDIT
        .stack_capturing_macro => {
            try renderToken(r, main_tokens[node], .none);
            try renderIdentifier(r, datas[node].lhs, .none, .preserve_when_shadowing);
            try renderToken(r, main_tokens[node] + 2, .space);
            try renderExpression(r, datas[node].rhs, space);
        },
        // END EDIT
...
fn renderCall(
    ...
    // BEGIN EDIT
    if (r.fixups.render_to_zig) {
        var has_stack_capturing_macro: bool = false;
        var stack_capturing_macro_param_idx: usize = 0;
        for (call.ast.params, 0..) |param, i| {
            const param_tag = r.tree.nodes.items(.tag)[param];
            if (param_tag == .stack_capturing_macro) {
                if (has_stack_capturing_macro) unreachable;
                has_stack_capturing_macro = true;
                stack_capturing_macro_param_idx = i;
            }
        }

        if (has_stack_capturing_macro) {
            const scm_data = r.tree.nodes.items(.data)[call.ast.params[stack_capturing_macro_param_idx]];
            const scm_name = scm_data.lhs;
            const scm_expr = scm_data.rhs;
            const block_index = r.disambiguator;
            r.disambiguator += 1;
            try r.ais.writer().print("_{d}: {{ const ", .{block_index});
            ...
            try r.ais.writer().print("; break :_{d} ", .{block_index});
            try renderIdentifier(r, scm_name, .none, .preserve_when_shadowing);
            try r.ais.writer().print(".end(", .{});
            try renderExpression(r, scm_expr, .none);
            try r.ais.writer().print(");}}", .{});
            switch (space) {
                .none => {},
                .semicolon => {
                ...
                else => std.debug.panic("TODO print space type: {s}", .{@tagName(space)}),
            }
            return;
        }
    }
...
fn renderParamList(
    ...
    return renderParamList_internal(r, lparen, params, space, if (params.len > 0) params[params.len - 1] else null);
}

fn renderParamList_internal(
    r: *Render,
    lparen: Ast.TokenIndex,
    params: []const Ast.Node.Index,
    space: Space,
    true_last_param: ?Ast.Node.Index,
) Error!void {
    // END EDIT
    const tree = r.tree;
    const ais = r.ais;
    ...
    // BEGIN EDIT
    const last_param = true_last_param.?;
    // END EDIT
