...
fn parseStackCapturingMacroExpr(p: *Parse, pipe_token: Node.Index) !Node.Index {
    const name = try p.expectToken(.identifier);
    _ = try p.expectToken(.pipe);

    return p.addNode(.{
        .tag = .stack_capturing_macro,
        .main_token = pipe_token,
...
fn parseSuffixExpr(p: *Parse) !Node.Index {
    ...
    while (true) {
        ...
        defer p.scratch.shrinkRetainingCapacity(scratch_top);
        // BEGIN EDIT
        var has_stack_capturing_macro = false;
        // END EDIT
        while (true) {
            if (p.eatToken(.r_paren)) |_| break;

            // BEGIN EDIT
            if (has_stack_capturing_macro) try p.warn(.arg_after_stack_capturing_macro);
            if (p.eatToken(.pipe)) |pipe_token| {
                const param = try p.parseStackCapturingMacroExpr(pipe_token);
                try p.scratch.append(p.gpa, param);
