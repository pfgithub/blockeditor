...
pub fn main() !void {
    ...
    if (stdin_flag) {
        ...
        if (check_ast_flag) {
            // BEGIN EDIT
            if (true) fatal("check ast is not available", .{});
            // END EDIT
            ...
            // BEGIN EDIT
            if (true) fatal("ast error printing is not available", .{});
            // END EDIT
...
fn fmtPathDir(
    ...
    while (try dir_it.next()) |entry| {
        ...
        // BEGIN EDIT
        if (is_dir or entry.kind == .file and ((mem.endsWith(u8, entry.name, ".zig") and !mem.endsWith(u8, entry.name, ".zigx.zig")) or mem.endsWith(u8, entry.name, ".zon") or mem.endsWith(u8, entry.name, ".zigx"))) {
            // END EDIT
...
fn fmtPathFile(
    ...
    if (fmt.check_ast) {
        // BEGIN EDIT
        if (true) fatal("check_ast is not available", .{});
        // END EDIT
